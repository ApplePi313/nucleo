...
void timer_restart(uint8_t number) { // this is only for tim6 and 7 right now
  // TIM 2-7 are at 0x4000 0000 to 0x4000 17FF
  // event generation register has an offset of 0x14

  * (volatile uint32_t *) (0x40000000 + (number * 0x400) + 0x14) |= 1U;
}
...
void clock_timer(uint8_t number, bool enable) {
  ...
  * (volatile uint32_t *) (0x40021000 + 0x4C) &= ~(1U << (number - 2));
  * (volatile uint32_t *) (0x40021000 + 0x4C) |= enable << (number - 2);
}
